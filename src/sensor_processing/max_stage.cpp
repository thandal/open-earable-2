...
MaxStage::MaxStage(enum ParseType input_type, size_t window_length)
...
int MaxStage::process(const struct sensor_data *const input[], struct sensor_data *output) {
    ...
    *output = *input[0];
    const float result = *std::max_element(samples.begin(), samples.end());
    return encode_minmax_result(parse_type, result, output);
}

void MaxStage::set_window_length(size_t new_window_length) {
...
size_t MaxStage::get_window_length() const {
